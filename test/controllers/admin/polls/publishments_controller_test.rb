...
class Admin::Polls::PublishmentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'admin publishes a poll' do
    ...
    assert_changes -> { poll.state }, from: :draft, to: :published do
      post admin_poll_publishment_path(poll)
      poll.reload
    end
  ...
  test 'non-admin cannot publish a poll' do
    ...
    assert_no_changes -> { poll.state } do
      assert_raise(Pundit::NotAuthorizedError) { post admin_poll_publishment_path(poll) }
      poll.reload
    end
  end
