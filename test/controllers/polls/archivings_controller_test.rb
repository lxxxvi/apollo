...
class Polls::ArchivingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'non-admin cannot archive archivable poll' do
    ...
    assert_no_changes -> { poll.state } do
      assert_raise(ActiveRecord::RecordNotFound) { post poll_archiving_path(poll) }
      poll.reload
    end
  end
  ...
  test 'guest cannot archive archivable poll' do
    ...
    assert_no_changes -> { poll.state } do
      assert_raise(Pundit::NotAuthorizedError) { post poll_archiving_path(poll) }
      poll.reload
    end
  end
end
