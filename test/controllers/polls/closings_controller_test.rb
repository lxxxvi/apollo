...
class Polls::ClosingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'non-admin cannot close closable poll' do
    ...
    assert_no_changes -> { poll.state } do
      assert_raise(ActiveRecord::RecordNotFound) { post poll_closing_path(poll) }
      poll.reload
    end
  end
  ...
  test 'guest cannot close closable poll' do
    ...
    assert_no_changes -> { poll.state } do
      assert_raise(Pundit::NotAuthorizedError) { post poll_closing_path(poll) }
      poll.reload
    end
  end
end
